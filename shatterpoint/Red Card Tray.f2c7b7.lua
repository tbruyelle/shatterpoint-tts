function onLoad(saved_data)
    ...
    ownerTint = { r = 0.862745, g = 0.101961, b = 0.090196}
...
function snapToSlot(zone, object)
    ...
    for _, snap in ipairs(snapPoints) do
        ...
        if object.hasTag(snapTags[1]) and object.hasTag(snapTags[2]) then
            ...
            object.setRotation(snap.rotation + Vector(0, 180, 0))
            break
        end
    end
end
...
function getUnitCard(unit, unitSquad)
    ...
    local cardPositions = {
        { squadIndex = 1, cardType = "primary", position = { -16.5, 0.2, 3 } },
        { squadIndex = 1, cardType = "secondary", position = { -10, 0.2, 3 } },
        { squadIndex = 1, cardType = "support", position = { -5, 0.2, 3 } },
        { squadIndex = 2, cardType = "primary", position = { 5, 0.2, 3 } },
        { squadIndex = 2, cardType = "secondary", position = { 10, 0.2, 3 } },
        { squadIndex = 2, cardType = "support", position = { 15, 0.2, 3 } }
    }
...
function getUnitModels(unit, object, deck, clonedDeck)
    ...
    for index, model in ipairs(unitModels) do
        ...
        local modelPosition = {
            object.getPosition()[1] - size.x/2 + 1 + modelOffset,
            object.getPosition()[2],
            object.getPosition()[3] + 5.5
        }
...
function getOrderCards(object, orderName, deck, clonedDeck)
	...
	if foundCard then
		...
		local orderCard = clonedDeck.takeObject({
			position = {
				object.getPosition()[1],
				object.getPosition()[2] + 0.2,
				object.getPosition()[3] - 4
			},
			rotation = {0, 180, 0},
			guid = cardGUID,
			smooth = false
...
function getUnitDurabilityTokens(unit, zone, object)
	...
	for i = 1, unitDurability do
		local tokenPosition = {
			objectPosition[1] + (objectSize.x/2) + 0.5,
			objectPosition[2],
			objectPosition[3] - (objectSize.z/2 + 0.5) + (i * tokenOffset)
		}

		-- Take a durability token from the durability bag  
		...
		durabilityToken.setPosition(tokenPosition)
